...
#         logpt = -nn.functional.cross_entropy(inputs, targets, weight=self.weight, reduction='none')
#         pt = torch.exp(logpt)
...
class EmotionRecognitionCNN(nn.Module):
        ...
        for epoch in range(1, epochs + 1):
            ...
            with torch.no_grad():
                for Xv, yv in val_loader:
                    ...
                    val_correct += (outv.argmax(1) == yv).sum().item()
    ...
    def test_model(self, test_loader, device='cpu', class_names=None):
        ...
        if class_names is None:
            class_names = [str(i) for i in range(self.model.fc.out_features)]
        print('\nClassification Report:')
        print(classification_report(all_labels, all_preds, target_names=class_names))
        return acc
...
def main():
    ...
    model.test_model(test_loader, device=device, class_names=classes)
